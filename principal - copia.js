	class Bola {
		constructor(radio, color, x, y, shoot, velX, velY){
			...
			if(this.shoot == 1){
				...
				this.circ.setAttributeNS('http://www.w3.org/1999/xlink','href', 'snowball.png');
	...
	window.onload = () =>{
		...
		var jugador = new Jugador("skyblue");
	...
	class Jugador {
		...
		mover(tecla) {
			...
			if ((tecla.keyCode === 97 && this.x - this.velX >= 0) || (tecla.keyCode === 65 && this.x - this.velX >= 0)) {
				...
				this.rect.setAttributeNS('http://www.w3.org/1999/xlink','href', 'santaizq.gif');
			}
			if ((tecla.keyCode === 100 && this.x + this.velX <= document.getElementById("svg").getBoundingClientRect().width - 50)) {
				this.x = this.x + this.velX;
				this.rect.setAttribute("x", this.x);
				this.rect.setAttributeNS('http://www.w3.org/1999/xlink','href', 'santader.gif');
			}
			if (tecla.keyCode === 32) {
				this.rect.setAttributeNS('http://www.w3.org/1999/xlink','href', 'shoot.png');
				var bolita = new Bola(10,"red",this.x +25, this.y, 1)
				document.getElementById("audio2").play();
				console.log(bolita.x);
				disparos.push(bolita);
				setTimeout(() => { this.rect.setAttributeNS('http://www.w3.org/1999/xlink','href', 'normal.png'); }, 200);
			}
